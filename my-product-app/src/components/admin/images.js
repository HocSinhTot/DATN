...
const ImageManagement = () => {
    ...
    const confirmDelete = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:8080/api/admin/images/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 200 || response.status === 201) {
                alert(popup.type === 'edit' ? 'Cập nhật hình ảnh thành công!' : 'Thêm hình ảnh thành công!');
            }
            ...
            console.error(`Error ${popup.type === 'edit' ? 'updating' : 'adding'} image:`, error);
        }
    };
                    ...
                    <div className="card">
                        <div className="card-header">
                            ...
                            <button
                                ...
                                onClick={() => setPopup({ show: true, type: 'add', image: null })}
                                style={{
                                    marginTop: '18px',
                                    ...
                                    {image.length > 0 ? (
                                        image.map((image, index) => (
                                            <tr key={image.id}>
                                                ...
                                                <td style={{ padding: '15px', textAlign: 'center' }}>
                                                    <button
                                                        onClick={() => setPopup({ show: true, type: 'edit', image })}
                                                        style={{
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h3 style={{ marginBottom: '25px', fontSize: '22px', fontWeight: 'bold' }}>
                            {popup.type === 'edit' ? 'Sửa hình ảnh' : popup.type === 'delete' ? 'Xác nhận xóa' : 'Thêm hình ảnh'}
                        </h3>
                        ...
                        {popup.type !== 'delete' ? (
                            ...
                            <div className="form-group">
                                <label htmlFor="image" style={{ fontWeight: 600, fontSize: '20px' }}>Chọn hình ảnh</label>
                                <input
                                    type="file"
                                    id="image"
                                    name="image"
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px' }}>
                                    <button className="btn btn-danger" onClick={() => confirmDelete(popup.image.id)}>
                                        Xóa
                                    </button>
